...
function AnahtardanDegerlendirmeAl(degerlendirmeler, index) {
  ...
  const metin = `${degerlendirme.isim} isimli kişi ${degerlendirme.puan} puan verdi ve şunları yazdı: ${degerlendirme.geribildirim}`;
...
function SonDegerlendirmeyiAl(degerlendirmeler) {
  ...
  const metin = `${sonDegerlendirme.isim} isimli kişi ${sonDegerlendirme.puan} puan verdi ve şunları yazdı: ${sonDegerlendirme.geribildirim}`;
...
function PuanaGoreDegerlendirmeAl(degerlendirmeler, puan) {
  const filtrelenmisDEgerlendrimeler = [];

  for (let i = 0; i < degerlendirmeler.length; i++) {
    const degerlendirme = degerlendirmeler[i];

    if (degerlendirme.puan >= puan && degerlendirme.puan < puan + 1) {
      filtrelenmisDegerlendirmeler.push(degerlendirme);
    }
    return filtrelenmisDEgerlendrimeler;
  }
}
...
function UzunDegerlendirmeleriAl(degerlendirmeler) {
  ...
  for (let i = 0; i < degerlendirmeler.length; i++) {
    const degerlendirme = degerlendirmeler[i];
    const geribildirimKelimeler = degerlendirme.geribildirim.split(" ");

    if (geribildirimKelimeler.length > 15) {
      uzunDegerlendirmeler.push(degerlendirme);
    }
  }
...
function arabaYapici(baslangicKilometresi) {
  const araba = {
    kilometreSayaci: baslangicKilometresi,
